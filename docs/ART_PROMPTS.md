...
> **画幅**：游戏与 BG / 定镜为 **横屏 16:9（1920×1080）**；角色 SK 为透明立绘（单人竖构图即可）。
...
Edge of red plastic running track visible along the bottom, simple beige campus buildings
...
Horizontal composition 16:9 landscape widescreen, camera at student eye level
standing in formation looking across the empty parade ground.
Documentary photography, natural colors, not HDR, not cinematic teal-orange.
```

**Gemini 补充说明（粘贴到对话）：**
> 中国大学军训操场，九月上午，写实照片。不要任何人物。横图 16:9。暖黄怀旧色调。

**验收**：无人、阳光从右上、地面有跑道线、远景教学楼虚化、横屏构图。
...
**画幅**：单人全身竖构图、白底；导出后去白为透明 PNG（叠在横屏 BG 上）
...
Ears may be visible. Face blank and smooth — NO eyes, nose, or mouth.
...
body leaning ONLY SLIGHTLY sideways — mild unsteady sway, knees barely bent,
arms slightly out for balance (not a dramatic fall).
...
2. SK 去白底：「选择 → 颜色范围 → 白色」删底，或混合模式「正片叠底」（勿删角色脸部留白）
3. 线稿统一：`图像 → 调整 → 色相/饱和度` 把线条倾向 `#7a756c`
4. 导出 PNG-24，勾选「透明度」
5. BG 导出 **1920×1080** JPG
...
3. 打开 `tools/art-align.html`（16:9 画布）微调坐标 → 更新 `assets/hotspots/scene1.json`
